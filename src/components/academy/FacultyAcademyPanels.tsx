...
interface CourseSection { id: string; code?: string; title?: string; description?: string; }
interface Assignment { id: string; section_id: string; title: string; description?: string; due_at?: string; max_points?: number; }
interface Announcement { id: string; section_id?: string; title: string; body?: string; published_at?: string; }
...
export const FacultyAcademyPanels: React.FC = () => {
  ...
  useEffect(() => {
    const load = async () => {
      setLoading(true);
      try {
        const { data: secData } = await supabase.from("course_sections").select("id, code, title, description").order("title");
        setSections(secData || []);
        const { data: assignData } = await supabase.from("assignments").select("id, section_id, title, description, due_at, max_points").order("due_at");
        setAssignments(assignData || []);
        const { data: annData } = await supabase.from("announcements").select("id, section_id, title, body, published_at").order("published_at", { ascending: false });
        setAnnouncements(annData || []);
        const { data: resData } = await supabase.from("section_resources").select("id, section_id, title, url").order("title");
  ...
  const refreshSections = async () => {
    const { data } = await supabase.from("course_sections").select("id, code, title, description").order("title");
    setSections(data || []);
  };
  const refreshAssignments = async () => {
    const { data } = await supabase.from("assignments").select("id, section_id, title, description, due_at, max_points").order("due_at");
    setAssignments(data || []);
  };
  const refreshAnnouncements = async () => {
    const { data } = await supabase.from("announcements").select("id, section_id, title, body, published_at").order("published_at", { ascending: false });
    setAnnouncements(data || []);
  };
  const refreshResources = async () => {
    const { data } = await supabase.from("section_resources").select("id, section_id, title, url").order("title");
    setResources(data || []);
  };
          ...
          <CardContent>
            ...
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-3">
                ...
                <Field label="Title"><Input value={newSection.title || ""} onChange={e => setNewSection(s => ({ ...s, title: e.target.value }))} /></Field>
                <Field label="Description"><Textarea value={newSection.description || ""} onChange={e => setNewSection(s => ({ ...s, description: e.target.value }))} /></Field>
              ...
              <div className="space-y-3">
                {sections.map(sec => (
                  <div key={sec.id} className="border rounded p-3 space-y-2">
                    <div className="text-sm font-medium">{sec.code} — {sec.title}</div>
                    <div className="text-xs text-muted-foreground">{sec.description}</div>
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="secondary" onClick={() => handleUpdate("course_sections", sec.id, { title: sec.title, code: sec.code, description: sec.description }, refreshSections)}><Save className="w-4 h-4 mr-1" /> Save</Button>
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-3">
                ...
                <Field label="Due At (ISO)"><Input value={newAssignment.due_at || ""} onChange={e => setNewAssignment(a => ({ ...a, due_at: e.target.value }))} /></Field>
              ...
              <div className="space-y-3">
                {assignments.map(a => (
                  <div key={a.id} className="border rounded p-3 space-y-2">
                    ...
                    <div className="text-xs text-muted-foreground">Due: {a.due_at || "—"} • Max: {typeof a.max_points === "number" ? a.max_points : "—"}</div>
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="secondary" onClick={() => handleUpdate("assignments", a.id, { title: a.title, description: a.description, due_at: a.due_at, max_points: a.max_points }, refreshAssignments)}><Save className="w-4 h-4 mr-1" /> Save</Button>
      ...
      <TabsContent value="announcements">
        <Card>
          ...
          <CardContent>
            <div className="grid md:grid-cols-2 gap-4">
              <div className="space-y-3">
                <Field label="Section ID (optional)"><Input value={newAnnouncement.section_id || ""} onChange={e => setNewAnnouncement(a => ({ ...a, section_id: e.target.value }))} /></Field>
                <Field label="Title"><Input value={newAnnouncement.title || ""} onChange={e => setNewAnnouncement(a => ({ ...a, title: e.target.value }))} /></Field>
                <Field label="Body"><Textarea value={newAnnouncement.body || ""} onChange={e => setNewAnnouncement(a => ({ ...a, body: e.target.value }))} /></Field>
              ...
              <div className="space-y-3">
                {announcements.map(a => (
                  <div key={a.id} className="border rounded p-3 space-y-2">
                    ...
                    <div className="text-xs text-muted-foreground">Section: {a.section_id || "Global"} • Published: {a.published_at || "—"}</div>
                    <div className="flex items-center gap-2">
                      <Button size="sm" variant="secondary" onClick={() => handleUpdate("announcements", a.id, { title: a.title, body: a.body }, refreshAnnouncements)}><Save className="w-4 h-4 mr-1" /> Save</Button>
